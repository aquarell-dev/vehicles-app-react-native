		...
		<View className='mt-2 mb-4'>
			...
			<View className='flex flex-row justify-around items-center'>
				<PrimaryButton onPress={() => Linking.openURL('tel:+12345')}>
					<Text className='font-medium' style={{ color: constants.colors.text.primary }}>{i18n.t('car.call')}</Text>
				</PrimaryButton>
				<SecondaryButton onPress={openWhatsAppDeepLink}>
					<Text className='font-medium' style={{ color: constants.colors.text.primary }}>{i18n.t('car.message')}</Text>
				</SecondaryButton>
			</View>
		</View>
	)
